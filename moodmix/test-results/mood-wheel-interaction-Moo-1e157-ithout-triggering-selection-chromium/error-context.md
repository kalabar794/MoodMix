...
- Name: Mood Wheel Interaction >> should show visual feedback on hover without triggering selection
- Location: /Users/jonathonc/Auto1111/claude/MoodMix/moodmix/tests/mood-wheel-interaction.spec.ts:36:7
...
Locator: locator('.mood-wheel-premium').first()
Expected: visible
Received: hidden
Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for locator('.mood-wheel-premium').first()
    9 × locator resolved to <div class="absolute inset-0 rounded-full mood-wheel-premium"></div>
      - unexpected value "hidden"

    at /Users/jonathonc/Auto1111/claude/MoodMix/moodmix/tests/mood-wheel-interaction.spec.ts:38:29
```
...
>  38 |     await expect(moodWheel).toBeVisible()
  ...
  105 |     
  106 |     // Check intensity is low (should be < 40%)
  107 |     await page.waitForSelector('text="Your Perfect Soundtrack"', { timeout: 10000 })
  108 |     const newIntensityText = page.locator('text=/\\d+%/').first()
  109 |     const newIntensity = await newIntensityText.textContent()
  110 |     const newIntensityValue = parseInt(newIntensity?.replace('%', '') || '0')
  111 |     expect(newIntensityValue).toBeLessThan(40)
  112 |   })
  113 | })
```